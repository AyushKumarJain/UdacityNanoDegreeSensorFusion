...
// In this lidar.h header file, lidar object is created. 
//It is needed to include it at the top of the environment.cpp file.
...
struct Lidar
{
	...
	double resoultion;
	double sderr;
    //  The ray object is defined as Lidar will use these rays to sense its surrounding by doing ray casting.
		...
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		...
		// Here, sderr is noise, it will help to develop more robust point processing functions.
		sderr = 0.2; //0.0 // noise will have scattered points
		cars = setCars;
    ...
    //  The scan function below in this Lidar struct will be doing the ray casting.
