...
// This function will output is expected to output &Boxes an object of std::vector<BoundingBox>
// nms is non minimum suppression
// confThreshold is the confidence threshold that Yolo will use to consider a classification
void detectObjects(cv::Mat& img, std::vector<BoundingBox>& bBoxes, const float confThreshold, const float nmsThreshold, 
                  const std::string basePath, const std::string classesFile, const std::string modelConfiguration, const std::string modelWeights, const bool bVis)
{
    ...
    while (getline(ifs, line)) 
        classes.push_back(line); // gets line from ifs (a ifstream object) and push_back into classes.
    ...
    net.setInput(blob);
    net.forward(netOutput, names); // Arguments as netOutput and the names of the output layers, Forward propogation will give out netOutput
