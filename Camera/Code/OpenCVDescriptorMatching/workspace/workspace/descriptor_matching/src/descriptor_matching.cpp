                      ...
                      vector<cv::DMatch> &matches, string descriptorType, string matcherType, string selectorType)
{
    ...
    // First checking if we want to use Brute Force or FLANN based matcher

    if (matcherType.compare("MAT_BF") == 0) // BF is Brute Force Matching, Computationaly expensive
    {
        // Second checking, if the Descriptors to match are encoded in Binary or Intensity gradient form
        int normType = descriptorType.compare("DES_BINARY") == 0 ? cv::NORM_HAMMING : cv::NORM_L2; // Depending of Descriptor type we had, i.e. Binary or Intensity gradient based, we will use, Hamming or L2.
        matcher = cv::BFMatcher::create(normType, crossCheck);
        cout << "BF matching cross-check=" << crossCheck;
    }
    else if (matcherType.compare("MAT_FLANN") == 0) // FLANN is Fast Library for approximate nearest neighbours, uses KD-tree data structure
    {
        if (descSource.type() != CV_32F)
        { // OpenCV bug workaround : convert binary descriptors to floating point due to a bug in current OpenCV implementation
            descSource.convertTo(descSource, CV_32F); // Source image Descriptor 
            descRef.convertTo(descRef, CV_32F); // Reference image Descriptor
        }
    ...
    // Matching can be with Nearest neighbour or K Nearest neighbour
